...
def detect_geometric_corners(polypath: Polypath,
    ...
    Второе условие (>= 45°) отсеивает:
      - биарк-шум (единицы градусов),
      - тангенциальные соединения на амёбах/почках (30° разворот 
        между прямой и дугой круга — это гладкий стык, а не угол).
    Настоящий острый угол ≥ 45° (например, 90° угол прямоугольника с 
    ...
    вокруг выпуклости или петля). Реальный тугой угол ≤ 180°.

    Args:
        polypath: контур ножа
        radius_threshold_mm: порог радиуса скругления (мм). По умолчанию 0.7.
        min_swept_deg: минимальный угол разворота дуги (град). По умолчанию 45.
