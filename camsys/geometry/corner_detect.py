...
def detect_geometric_corners(polypath: Polypath,
    ...
    Дуга считается углом, только если выполнены ОБА условия:
      1. собственный радиус дуги < radius_threshold_mm;
      2. дуга разворачивает касательную на >= min_swept_deg градусов.
    Второе условие отбрасывает крошечные биарк-дуги на гладких кривых
    (разворот единицы градусов), сохраняя настоящие тугие углы (десятки
    градусов). См. _arc_swept_deg.

    Args:
        polypath: контур ножа
        radius_threshold_mm: порог радиуса скругления (мм). По умолчанию 0.7.
        min_swept_deg: минимальный угол разворота дуги (град). По умолчанию 15.
    ...
    for i, seg in enumerate(polypath.segments):
        ...
        if _arc_swept_deg(seg) < min_swept_deg:
            continue
